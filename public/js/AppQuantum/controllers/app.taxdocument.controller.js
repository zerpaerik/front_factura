(function () {
  ...
  angular.module("QuantumApp").controller("TaxDocumentController", TaxDocumentController);

  TaxDocumentController.$inject = [
    "$scope",
    "$http",
    "SupplierService",
    "HomeService",
  ...
  function TaxDocumentController(
    $scope,
    $http,
    SupplierService,
    HomeService,
    ...
    vm.SelectTaxTypeCode = function (data, param) {
      if (data.taxtypecode.id === 109) vm.showRenta = "1";
      else vm.showRenta = "";
    ...
    vm.addTaxDocumentLine = function (taxDocument) {
      ...
      if (
        vm.referralDocumentTypeSelected === undefined ||
        vm.referralDocumentTypeSelected === null ||
        vm.referralDocumentTypeSelected === ""
      ) {
      ...
      if (taxDocument.referral_document.length < 13) {
        SweetAlert.swal(
          "Linea de Retención!",
          "Debe ingresar un Nro. de Comprobante válido!",
          "warning",
        );
        return;
      }
      ...
      if (taxDocument.doc_emission_date === "" || taxDocument.doc_emission_date === undefined) {
        SweetAlert.swal(
          ...
          "La Fecha de Emisión debe ser seleccionada!",
          "warning",
        );
        return;
      }
      ...
      if (taxDocument.doc_emission_date > vm.FechaHoy) {
        SweetAlert.swal(
          "Fecha de Emisión Incorrecta!",
          "La Fecha de Emisión debe ser menor a la Fecha Actual!",
          "warning",
        );
        return;
      }
      ...
      if (taxDocument.tax_base_amount <= 0) {
        SweetAlert.swal(
          ...
          "La Base Imponible debe ser mayor!",
          "warning",
        );
        return;
      }
      ...
      if (isNaN(parseFloat(taxDocument.tax_base_amount))) {
        SweetAlert.swal(
          "Base Imponible incorrecta!",
          "La Base Imponible acepta solo valores numericos!",
          "warning",
        );
        return;
      }
      ...
      if (taxDocument.taxtypecode.id != 109) {
        ...
        if (taxDocument.taxpercentage === null || taxDocument.retentiontypecode === undefined) {
          SweetAlert.swal(
            ...
            "Debe seleccionar el Porcentaje de Retención!",
            "warning",
          );
          return;
        }
        ...
        if (taxDocument.tax_percentage === "" || taxDocument.retentiontypecode === undefined) {
          SweetAlert.swal(
            "Porcentaje de Retención!",
            "Porcentaje de Retención debe ser mayor o igual a cero!",
            "warning",
          );
          return;
        }
      ...
      promise.then(
        function (pl) {
          toastr.success(
            ...
            "Se ha modificado la Retención satisfactoriamente",
          );
          if (enviar) {
            vm.validateSRI(id, 1);
          } else window.location = HOST_ROUTE.TAXDOCUMENT;
        },
        promise.catch(function (reason) {
          console.log(reason);
        }),
    ...
    vm.createSupplier = function () {
      if (vm.toggleSelected == true) vm.supplier.is_active = 1;
      else vm.supplier.is_active = 0;
