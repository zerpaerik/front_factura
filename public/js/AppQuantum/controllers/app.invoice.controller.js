(function () {
  ...
  angular.module("QuantumApp").controller("InvoiceController", InvoiceController);
  ...
  function InvoiceController(
            ...
            if (preinvoice != null && preinvoice == 1) {
              ...
              addCorrelativePromise.then(function (addPl) {
                if (addPl.data == 1) {
                  ...
                  SweetAlert.swal(
                    "Hubo un error al enviar al SRI",
                    "" + "No se pudo asociar un correlativo a la prefactura",
                    "error",
                  );
                }
              });
    ...
    vm.edit = function (id, user) {
      ...
      promise.then(function (pl) {
        ...
        if (pl.data.export_invoice === 1) vm.ExportInvoicetoggleSelected = true;
        else vm.ExportInvoicetoggleSelected = false;
    ...
    vm.calculateTip = function () {
      if (vm.tiptoggleSelected) {
        vm.Invoice.tip = vm.totalInvoiceNoTax * 0.1;
    ...
    vm.updateInvoice = function () {
      ...
      if (vm.Invoice.export_invoice === 1) {
        if (isNaN(parseInt(vm.Invoice.international_cargo))) {
          SweetAlert.swal(
            "Flete Internacional!",
            "Flete Internacional acepta valores numericos!",
            "warning",
          );
          return;
        }
        ...
        if (isNaN(parseInt(vm.Invoice.international_secure))) {
          SweetAlert.swal(
            ...
            "Seguro Internacional acepta valores numericos!",
            "warning",
          );
          return;
        }
        ...
        if (isNaN(parseInt(vm.Invoice.custom_expenditures))) {
          SweetAlert.swal(
            "Gastoas Aduaneros!",
            "Gastos Aduaneros acepta valores numericos!",
            "warning",
          );
          return;
        }
        ...
        if (isNaN(parseInt(vm.Invoice.transport_expenditures))) {
          SweetAlert.swal(
            ...
            "Gastos de Transporte acepta valores numericos!",
            "warning",
          );
          return;
        }
        ...
        if (vm.ExportInvoicetoggleSelected) {
          vm.totalInvoice +=
            parseFloat(vm.Invoice.international_cargo) +
            parseFloat(vm.Invoice.international_secure) +
            parseFloat(vm.Invoice.custom_expenditures) +
            parseFloat(vm.Invoice.transport_expenditures);
        }
      }
    ...
    vm.addInvoiceLine = function (product) {
      ...
      if (isNaN(parseInt(product.quantity))) {
        SweetAlert.swal(
          "Cantidad incorrecta!",
          "La Cantidad acepta solo valores numericos!",
          "warning",
        );
        return;
      }
      ...
      if (isNaN(parseInt(product.discount))) {
        SweetAlert.swal(
          ...
          "El Descuento acepta solo valores numericos!",
          "warning",
        );
        return;
      }
      ...
      if (product.expired_date !== "" && product.expired_date !== undefined)
        expired_date =
          product.expired_date.substring(8) +
          "/" +
    ...
    vm.addInvoicePayment = function (payment) {
      ...
      if (payment.mount <= 0) {
        SweetAlert.swal(
          "Monto incorrecto!",
          "El monto de Pago no puede ser menor o igual a cero!",
          "warning",
        );
        return;
      }
      ...
      if (isNaN(parseFloat(payment.mount))) {
        SweetAlert.swal(
          "Monto incorrecto!",
          "El Monto del pago acepta solo valores numéricos!",
          "warning",
        );
        return;
      }
      ...
      if (mount_a <= vm.totalInvoice) {
        ...
        SweetAlert.swal(
          "Monto Excedido!",
          "El monto de Pago excede el Total de Factura!",
          "warning",
        );
      }
    };
    ...
    vm.createPreInvoice = function () {
      ...
      if (vm.ExportInvoicetoggleSelected) vm.Invoice.export_invoice = 1;
      else vm.Invoice.export_invoice = 0;
    ...
    vm.updatePreInvoice = function (id, enviar = null) {
      ...
      promise.then(
        function (pl) {
          toastr.success(
            "Factura actualizada",
            "Se ha modificado la Factura Proforma satisfactoriamente",
          );
          if (enviar) {
            vm.validateSRI(id, 1, 1);
          } else window.location = HOST_ROUTE.PREINVOICE;
        },
        promise.catch(function (reason) {
          console.log(reason);
        }),
    ...
    vm.createClient = function () {
      if (vm.toggleSelected == true) vm.client.is_active = 1;
      else vm.client.is_active = 0;
