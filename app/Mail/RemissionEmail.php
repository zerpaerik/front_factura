...
class RemissionEmail extends Mailable
{
    ...
    {   
        ...
        if(isset($this->demo->xml_file)){
            return $this->from('no.reply.quantumapp@gmail.com')
                    ...
                    ->view('mails.remission', compact('body', 'legend', 'mailResponse', 'companyName'))                    
                    ->text('mails.remission_plain', compact('body', 'legend', 'mailResponse', 'companyName'))                    
                    ->attach($this->demo->pdf_file, ['as' => 'remissionGuide.pdf', 'mime' => 'application/pdf'])
                    ->attach($this->demo->xml_file, ['as' => 'remissionGuide.xml', 'mime' => 'text/xml']);
        }
        else{
            return $this->from('no.reply.quantumapp@gmail.com')
                    ->subject($emailObject->subject)
