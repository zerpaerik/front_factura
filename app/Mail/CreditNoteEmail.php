    ...
    {   
        ...
        if(isset($this->demo->xml_file)){
            return $this->from('no.reply.quantumapp@gmail.com')
                    ...
                    ->view('mails.creditnote', compact('body', 'legend', 'mailResponse', 'companyName'))                    
                    ->text('mails.creditnote_plain', compact('body', 'legend', 'mailResponse', 'companyName'))                    
                    ->attach($this->demo->pdf_file, ['as' => 'creditNote.pdf', 'mime' => 'application/pdf'])
                    ->attach($this->demo->xml_file, ['as' => 'creditNote.xml', 'mime' => 'text/xml']);
        }
        else{
            return $this->from('no.reply.quantumapp@gmail.com')
                    ->subject($emailObject->subject)
