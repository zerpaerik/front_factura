    ...
    {   
        ...
        if(isset($this->demo->xml_file)){
            return $this->from('no.reply.quantumapp@gmail.com')
                    ...
                    ->view('mails.invoice', compact('body', 'legend', 'mailResponse', 'companyName'))                    
                    ->text('mails.invoice_plain', compact('body', 'legend', 'mailResponse', 'companyName'))                    
                    ->attach($this->demo->pdf_file, ['as' => 'invoice.pdf', 'mime' => 'application/pdf'])
                    ->attach($this->demo->xml_file, ['as' => 'invoice.xml', 'mime' => 'text/xml']);
        }
        else{
            return $this->from('no.reply.quantumapp@gmail.com')
                    ->subject($emailObject->subject)
